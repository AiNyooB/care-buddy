---
name: PackageHero 套餐包区域重构 — Chip 居中 → 选中 morph 主卡 + 右侧次 chip 堆叠（v4：标题居中，位于 chip 行上方）
overview: 将 src/components/PackageHero.tsx 重构为：常驻「今日推荐」主标题位于 chip 行 / 主卡行的正上方（中部，非最顶部）、默认态 3 个套餐 chip 横向平铺居中、点击后选中 chip 通过 shared layout 渐变成主套餐卡片（圆角/阴影/padding 插值）、另两个未选中 chip 用 FLIP 滑到主卡右侧垂直堆叠，背景层（3D 蒙版图 crossfade）与文字云层（17 个并集运动）作为氛围辅助，仅手动点击切换，整体柔和 0.35-0.45s 过渡。
design:
  architecture:
    framework: react
    component: shadcn
...
todos:
  ...
  - id: rewrite-package-hero
    content: 重写 src/components/PackageHero.tsx 为三层结构（背景 + 文字云 + 中部编排交互层），含主标题「今日推荐」位于编排行正上方、默认 selectedPackageId='package-standard'、chip 默认态居中横排 / 选中态主卡 + 次 chip 右 stack 由 motion layout 自动驱动
    status: completed
    dependencies:
...
重构 `ExerciseLibrary` 顶部的套餐 Hero 区域。整体为常驻氛围层 + 中部编排交互区的两层结构：

- **常驻氛围层（z-0/10）**：底层 3D 蒙版图（弓步 + 树式，opacity 0.5，呼吸缩放）+ 中层 17 个并集运动名称文字云（按当前选中套餐染色），不参与形态变化
- **中部编排交互区（z-20）**：由「今日推荐」主标题 + 编排行组成
- **主标题**位于编排行（chip 行 / 主卡行）**正上方**（中部区域）
- **默认态**：3 个套餐名称 chip 横向平铺居中（中间为默认主选）
- **选中态**：被点击 chip morph 成主套餐卡片（共享元素动画 + 内部文本 stagger），另两个未选 chip FLIP 滑到主卡右侧垂直堆叠

## 核心功能

- **常驻主标题（位于 chip 行正上方）**：编排行顶部显示「今日推荐」（i18n key `package.sectionTitle`）
- **默认形态**：3 个套餐 chip（仅名称）横向平铺居中，初始选中 index=1（起身唤醒包），默认选中 chip 视觉稍强化
- **选中形态**：被点击 chip morph 成主套餐卡片（圆角/padding/阴影同步插值），主卡占据主区（flex-1 宽度），另两个 chip FLIP 滑到主卡右侧垂直堆叠
- **形态切换动效**：
- chip → 卡片：`motion` `layoutId` 共享元素动画，0.35s easeInOut，圆角/阴影/padding 通过 inline `style` 插值
...
3. **中部编排交互层**（z-20，居中布局，flex flex-col）：

- **主标题行**（常驻，居中）：`text-lg font-semibold text-foreground`，内容为 `t('package.sectionTitle')`
- **编排行**（由 `selectedPackageId` 驱动 layout 切换）：
    - 默认态：3 chip 横向平铺居中
    - 选中态：主套餐卡片（flex-1）+ 另 2 个 chip 在主卡右侧 `flex flex-col gap-2` 垂直堆叠

各层独立 `AnimatePresence`，key 为 `selectedPackageId`，实现"切换时各层同步过渡"。

## 关键实现策略

### 1. 容器布局（修正：主标题位于编排行正上方）

```
<div className="relative h-full overflow-hidden">          {/* 容器，背景层 + 文字云层 absolute 叠在内部 */}
  <PackageBackdrop ... />
  <ExerciseTagCloud ... />

  <div className="relative z-20 flex h-full flex-col items-center justify-center gap-4 p-4">
    <h2 className="text-lg font-semibold text-foreground">{t('package.sectionTitle')}</h2>   {/* 主标题：编排行正上方 */}
    <div className={isMainSelected ? 'flex w-full max-w-md items-stretch gap-3' : 'flex items-center gap-2'}>
      <PackageChip ... isMain />     {/* 选中套餐：主卡形态 */}
      <div className="flex flex-col gap-2">     {/* 未选套餐：右侧垂直 stack */}
        <PackageChip ... />
        <PackageChip ... />
      </div>
    </div>
...
return (
  <motion.button
    ...
    className={isMain
      ? 'flex-1 bg-card text-card-foreground border border-border text-left'
...
未选 chip 也用 `motion.button` + `layout`，当它从"中间位置"被重新布局到"右侧垂直 stack"时，`layout` 自动计算 FLIP 位移并插值。
...
- 容器高度 ≈ 240px（保持 `ExerciseLibrary` 中分配的高度），内部用 `flex flex-col items-center justify-center gap-4` 居中编排主标题 + 编排行
...
src/
├── components/
│   ├── PackageHero.tsx                    # [MODIFY] 重写为三层结构（~240 行）
│   ├── PackageChip.tsx                    # [NEW] chip 形态 + 主卡形态统一组件（~90 行），受控 props: pkg, isMain, onSelect
...
- Foreground：#1A1A1A（主卡标题、当前套餐的文字云标签、主标题）
...
当前窗口 492×696 固定，无需响应式断点；组件用 `relative h-full` 占满 PackageHero 父容器
...
整体走"低饱和 + 大留白 + 柔和动效 + 编排式形态变化"路线。底层两张 3D 白色人体剪影（弓步 + 树式）配 17 个并集运动名称文字云作为常驻氛围层，中部「今日推荐」主标题位于编排行（chip 行 / 主卡行）正上方，下方是"3 chip 居中 ↔ 1 主卡 + 2 次 chip 右 stack"的编排式形态。形态切换由 `motion` 的 `layoutId` 共享元素 + `layout` FLIP 驱动，0.35s 柔和过渡，圆角/阴影/padding 通过 style 同步插值。
...
3. **中部编排交互层**（z-20，居中布局）：
    ...
    - **主标题行**（常驻，居中）：`text-lg font-semibold text-foreground`，内容「今日推荐」，位于编排行**正上方**
    - **编排行**（由 `selectedPackageId` 驱动 layout 切换）：
