    ...
    return (
      <div className="flex flex-col items-center">
        <CircularProgress size={200} strokeWidth={8} progress={progress}>
          <span className="font-sans text-type-lock-timer text-white">
...
export function LockScreenSlave() {
  ...
  useEffect(() => {
    ...
    const fallbackMs = remaining > 0 ? remaining * 1000 : 30_000;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state, currentExerciseId, currentConfig, currentExerciseIndex, fullExerciseIds.length, remaining]);
