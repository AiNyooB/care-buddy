        ...
        if !state.is_frozen() {
            if let Some(freeze_start) = state.freeze_start.take() {
                ...
                for timer in state.tasks.values_mut() {
                    ...
                    if timer.triggered {
                        // Reset triggered tasks completely
                        timer.triggered = false;
                        timer.reset_time = Instant::now();
                        timer.snoozed = false;
                        timer.snooze_count = 0;
                    } else if timer.reset_time > freeze_start {
                        // 锁屏期间被重置的任务（reset_time 晚于冻结开始时间），不补偿
                        // 避免 timer_reset_task 在锁屏期间调用后，退出时被推迟整个冻结时长
                    } else {
                        timer.reset_time += freeze_duration;
                        // 如果任务被禁用，也需要同步更新 disabled_at，保持相对时间不变
                        if let Some(ref mut disabled_at) = timer.disabled_at {
        ...
        loop {
            ...
            // 锁屏状态看门狗：检测 slave 窗口是否全部被外部关闭
            {
                ...
                if is_locked_wd {
                    ...
                    if all_gone && !guard.windows.is_empty() {
                        ...
                        if !timer_state.is_frozen() {
                            timer_state.freeze_start = None;
                        }
...

            {
                ...
                if state.is_idle {
                    ...
                    // near-miss 扫描：将合并窗口内的未触发任务加入当前锁屏
                    if !tasks_to_trigger.is_empty() {
                        ...
                        for timer in state.tasks.values_mut() {
                            ...
                            if is_daily_task(&timer.config) {
                                let remaining = daily_remaining_seconds(&timer.config);
                                if remaining <= window {
            ...
            if let Some(first) = tasks_to_trigger.first() {
                // 合并同一 tick 的多任务触发 + near-miss 为一条 lock-screen-open
                let mut merged_ids: Vec<String> = tasks_to_trigger.iter().skip(1).map(|t| t.id.clone()).collect();
                merged_ids.extend(merged_near_ids);
...
fn ensure_floating_window(app: &AppHandle, visible_on_create: bool) -> Result<WebviewWindow, String> {
    ...
    // 水平居中贴顶
    let builder = if let Some(monitor) = app.primary_monitor().ok().flatten() {
        let monitor_size = monitor.size();
        let x = (monitor_size.width as f64 - 320.0) / 2.0;
        builder.position(x, 0.0)
    } else {
        builder
    };
...
fn exit_lock_mode(app: tauri::AppHandle, state: State<LockState>) {
    ...
    // Bug N9: 兜底重置 timer_state，防止前端未调用 timer_set_lock_screen_active(false) 导致卡死
    // 若前端正常调用 timer_set_lock_screen_active(false)，lock_screen_active 已为 false，此处跳过
    let mut ts = get_timer_state().lock().unwrap();
    if ts.lock_screen_active {
        ts.lock_screen_active = false;
        // 若处于冻结状态，执行补偿逻辑（与 timer_set_lock_screen_active(false) 一致）
        if let Some(freeze_start) = ts.freeze_start.take() {
            let freeze_duration = freeze_start.elapsed();
            for timer in ts.tasks.values_mut() {
                if timer.snoozed {
                    continue;
                }
                if timer.triggered {
