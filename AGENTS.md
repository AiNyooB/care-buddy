...
- **窗口** — 540×820，无边框（`decorations: false`），`withGlobalTauri: true`，`resizable: false`
- **布局** — 侧边栏 `--sidebar-width: 76px`，标题栏 `--titlebar-height: 76px`，主内容 `p-4`，三卡片区 `--card-area: calc(136px * 3 + 12px * 2)`
...
src/                      # React 前端
  ...
  hooks/useGuidedExercise.ts  # 引导锻炼状态机 Hook
